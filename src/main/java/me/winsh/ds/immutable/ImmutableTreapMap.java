...
public class ImmutableTreapMap {
	...
	static public interface Node<K, V> extends ImmutableTreapValue<K, V> {
	...
	static public class ExternalNode<K, V> implements Node<K, V> {
		...
		@SuppressWarnings("unchecked")
		public final boolean firstNodeTraverseRange(K lo, K hi, Consumer<K> consumer,
		...
		@SuppressWarnings("unchecked")
		public final boolean notFirstNodeTraverseRange(K lo, K hi, Consumer<K> consumer,
				Comparator<? super K> comparator) {
	...
	/*
	 * Stack should be empty when calling this function After the call the top of
	 * the stack will contain the root of a join of the two input trees (also
	 * returned as the return value). The returned stack contains the path to the
	 * returned node in the tree which has two external nodes as children and might
	 * need to be rotated to the right place.
	 */
	private static <K, V> InternalNode<K, V> joinHelper(Node<K, V> left, Node<K, V> right,
			@SuppressWarnings("rawtypes") Stack<Node> stack) {
	...
	@SuppressWarnings("unchecked")
	private static <K, V> Node<K, V> handleInsertedInternalNode(@SuppressWarnings("rawtypes") Stack<Node> stack,
			InternalNode<K, V> nodeClone) {
		Node<K, V> node = (Node<K, V>) stack.pop();
	...
	public static final <K, V> void traverseKeysInRange(ImmutableTreapValue<K, V> root, K lo, K hi,
	...
	private static final <K, V> void traverseKeysInRangeStack1(ImmutableTreapValue<K, V> root, K lo, K hi,
		...
		while (stack.size() != 0 || node != null) {
			if (node instanceof InternalNode) {
				InternalNode<K, V> nodeI = (InternalNode<K, V>) node;
				if (compare(hi, nodeI.getKey(), comparator) >= 0 /* greaterThanEqual(hi, node.key) */) {
					stack.push(node);
				}
				if (compare(lo, nodeI.getKey(), comparator) <= 0 /* lessThanEqual(lo, node.key) */) {
					node = nodeI.getLeft();
				} else {
					node = null;
				}
				...
				InternalNode<K, V> nodeI = (InternalNode<K, V>) node;
				if (nodeI != null
						&& compare(hi, nodeI.getKey(), comparator) >= 0 /* greaterThanEqual(hi, node.key) */) {
					node = nodeI.getRight();
				} else {
					node = null;
				}
	...
	public static <K, V> void traverseAllItems(ImmutableTreapValue<K, V> root, BiConsumer<K, V> consumer) {
		...
		while (stack.size() != 0 || node != null) {
			if (node instanceof InternalNode) {
				stack.push(node);
				node = ((InternalNode<K, V>) node).getLeft();
			} else {
				ExternalNode<K, V> nodeE = (ExternalNode<K, V>) node;
